...
std::vector<index_oid_t> CatalogAccessor::GetIndexes(table_oid_t table) {
...
index_oid_t CatalogAccessor::CreateIndex(namespace_oid_t ns, table_oid_t table, const std::string &name, IndexSchema *schema) {
  return dbc_->CreateIndex(txn_, ns, name, table, schema);
}

const IndexSchema &CatalogAccessor::GetIndexSchema(index_oid_t index) {
