...
class AttributeHelper {
 public:
  ...
  static std::unique_ptr<Column> MakeColumn(storage::ProjectedRow * pr, storage::ProjectionMap table_pm) {
    ...
    auto * col_expr = reinterpret_cast<const parser::AbstractExpression *>(pr->AccessForceNotNull(table_pm[ADBIN_COL_OID]));
  ...
  static storage::VarlenEntry CreateVarlen(const std::string & str) {
    ...
    if (str.size() > storage::VarlenEntry::InlineThreshold()) {
      ...
      varlen = storage::VarlenEntry::CreateInline(reinterpret_cast<const byte*>(str.data()), static_cast<uint32_t>(str.size()));
    }
    return varlen
  }
